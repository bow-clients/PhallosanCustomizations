...
use Shopware\Core\Checkout\Customer\Subscriber\CustomerGroupSubscriber;
...
#[Package('checkout')]
class CustomerGroupSubscriberDecorator extends CustomerGroupSubscriber
{
    ...
    public function __construct(
        public CustomerGroupSubscriber $decorated,
        public EntityRepository $customerGroupRepository,
        public EntityRepository $seoUrlRepository,
        public EntityRepository $languageRepository,
        public SeoUrlPersister $persister,
        public SlugifyInterface $slugify
    ) {
        parent::__construct(
        ...
        foreach ($groups as $group) {
            ...
            foreach ($group->getRegistrationSalesChannels() as $registrationSalesChannel) {
                ...
                foreach ($languageIds as $languageId) {
                    ...
                    // Generate own url from custom field
                    $customFields = $this->getTranslatedCustomFields($group->getTranslations(), $language);
                    $seoPathInfo = str_replace(' ', '', $customFields['customer_group_seo_url'] ?? '');
                    
                    // if the custom field is not set, the title is used
    ...
    // added new function
    private function getTranslatedCustomFields(?CustomerGroupTranslationCollection $translations, LanguageEntity $language): array
    {
        if ($translations === null) {
            return [];
        }
