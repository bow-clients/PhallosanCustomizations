...
class ImportVatCollector implements CartDataCollectorInterface
{
    use TaxHandlingTrait;

    final public const LINE_ITEM_EXTENSION_IMPORT_VAT = 'lineItemTaxRuleImportVat';
    ...
    public function collect(CartDataCollection $data, Cart $original, SalesChannelContext $context, CartBehavior $behavior): void
    {
        ...
        $lineItemImportVatMap = $this->createImportVatToLineItemIdMap($productLineItems, $context);
        if (empty($lineItemImportVatMap)) {
            return;
        }

        $original->addExtension(
            self::LINE_ITEM_EXTENSION_IMPORT_VAT,
            new ImportVatCartExtensionStruct($lineItemImportVatMap)
        );
    }
 ...
 */
    private function createImportVatToLineItemIdMap(
        LineItemCollection $lineItems,
        SalesChannelContext $context,
    ): array {
        $taxRuleImportVatMap = $this->createTaxRuleImportVatMap($context);
        $productEntities = $this->getLineItemProductEntities($lineItems, $context);

        /** @var array<string, ImportVatLineItemStruct[]> $importVatByLineItemId */
        $importVatByLineItemId = [];
        ...
        foreach ($lineItems as $lineItem) {
            ...
            foreach ($taxRules as $taxRule) {
                ...
                if (!isset($taxRuleImportVatMap[$taxRuleId])) {
                    continue;
                }

                // Shopware use the tax rate as key in line item tax rules
                $key = (string) ($taxRule->getTaxRate());
                $lineItemTaxRules[$key] = new ImportVatLineItemStruct(
                    $lineItem->getId(),
                    $taxRuleId,
                    $taxRuleImportVatMap[$taxRuleId]
                );
            }
            $importVatByLineItemId[$lineItem->getId()] = $lineItemTaxRules;
        }

        return $importVatByLineItemId;
    }

    /**
     * Get the TaxRuleEntities for the current SalesChannel with the ImportVatExtension.
     */
     ...
     */
    private function createTaxRuleImportVatMap(SalesChannelContext $context): array
    {
        $taxRuleCollection = $this->getSalesChannelTaxRuleEntities($context);

        /** @var array<string, float> $taxRuleImportVatMap */
        $taxRuleImportVatMap = [];
        ...
        foreach ($taxRuleCollection as $taxRule) {
            ...
            $applyImportVat = $taxRuleExtension->getApplyImportVat();

            $importVat = $taxRuleExtension->getImportVat() ?? 0.0;
            if (!$applyImportVat || $importVat <= 0) {
                continue;
            }

            $taxRuleImportVatMap[$taxRule->getId()] = $importVat;
        }

        return $taxRuleImportVatMap;
    }
